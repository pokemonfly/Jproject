    ...
    getExtraBtnGroup( record ) {
        ...
        return (
            <span className="keyword-extra-btn-dropdown show-on-row-hover">
                <Dropdown.Button
                    size="small"
                    onClick={this.onRowSeparateClick.bind( this, key )}
                    overlay={(
                    <Layout className="keyword-extra-btn-group">
                        <Tooltip title="修改匹配方式" arrowPointAtCenter="arrowPointAtCenter">
                            <Icon type="jingzhundingwei"/>
                        </Tooltip>
                        <Tooltip title={optimizeStatus ? '黑名单' : '该词优化方式为只优化价格，黑名单功能不可用'} arrowPointAtCenter="arrowPointAtCenter">
                            <Icon type="shanchuheimingdan"/>
                        </Tooltip>
                        <Tooltip title="删除关键词" arrowPointAtCenter="arrowPointAtCenter">
                            <Trigger popup={( <DelKeyword {...btnProps}/> )}>
                                <Icon type="shanchu"/>
                            </Trigger>
                        </Tooltip>
                        <Tooltip title="历史趋势" arrowPointAtCenter="arrowPointAtCenter">
                            <Icon type="qushi"/>
                        </Tooltip>
                        <Tooltip title="重点关注" arrowPointAtCenter="arrowPointAtCenter">
        ...
        let cols = [
            {
                ...
                render: ( text, record ) => {
                    ...
                    return <span>
                        ...
                        {record.get( 'wordscorelist' ) && record.getIn([ 'wordscorelist', 'wirelessQscore' ]) >= 6 && (
                            <Tooltip title="有机会在淘宝网电脑版搜索结果首页左侧推广位置展示（每天定时更新）" arrowPointAtCenter="arrowPointAtCenter">
                                <Icon type="zuo01"/>
                            </Tooltip>
                        )
}
                ...
                render: ( text, record ) => {
                    ...
                    return (
                        <span>
                            {pcAuto != -1 && (
                                <Tooltip title="抢排名词管理" arrowPointAtCenter="arrowPointAtCenter">
