...
export default class KeywordInfo extends Component {
    ...
    componentWillReceiveProps( props ) {
        ...
        if ( campaignMap.hasOwnProperty( campaignId ) ) {
            ...
            this.wordPriceLimit = this.campaign.wordPriceLimit == -1 ? divide( mobileWordMaxPrice, mobileDiscount / 100, 2 ) : this.campaign.wordPriceLimit
        }
    }
    optimizeChange() {}
    ...
    render() {
        ...
        const onlineStatusMenu = ( <Menu onClick={this.onOnlineStatusChange}>
            {onlineStatus != 'online' && ( <Menu.Item key="online">推广中</Menu.Item> )}
            {onlineStatus == 'online' && ( <Menu.Item key="offline">已暂停</Menu.Item> )}
        </Menu> )
        return ( <div className="keyword-info">
            <div className="left">
            ...
            <div className="right">
                ...
                <div className="row2">
                    ...
                    <span>PC限价：{wordMaxPrice == -1 ? this.wordPriceLimit: wordMaxPrice}</span>
                    <EditPriceModal {...this.props} editType='pc' title="PC限价" wordPriceLimit={this.wordPriceLimit}/>
                    <span>移动限价：{mobileWordMaxPrice}</span>
                    <EditPriceModal {...this.props} editType='mobile' title="移动限价"/>
                    <span>移动溢价：{mobileDiscount}</span>
                    <EditPriceModal {...this.props} editType='mobileDiscount' title="移动溢价"/>
                </div>
                <div className="row3">
